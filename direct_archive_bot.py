#!/usr/bin/env python3
"""
Direct Archive Bot - Uses archive.ph direct URL format
Creates archive URLs directly without parsing redirects
"""

import re
import requests
import json
from datetime import datetime

class DirectArchiveBot:
    def __init__(self):
        self.bot_username = "Angel_Dimi_Bot"
        self.processed_file = "processed_messages.json"
        self.processed_messages = self.load_processed_messages()
    
    def load_processed_messages(self):
        """Load previously processed message IDs"""
        try:
            with open(self.processed_file, 'r') as f:
                return set(json.load(f))
        except FileNotFoundError:
            return set()
    
    def save_processed_messages(self):
        """Save processed message IDs"""
        with open(self.processed_file, 'w') as f:
            json.dump(list(self.processed_messages), f)
    
    def extract_urls(self, text):
        """Extract URLs from text"""
        if not text:
            return []
        
        # Match http/https URLs
        url_pattern = r'http[s]?://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\\(\\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+'
        urls = re.findall(url_pattern, text)
        
        # Match www. URLs
        www_pattern = r'(?:^|\s)(www\.(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\\(\\),])+)'
        www_matches = re.findall(www_pattern, text)
        
        # Add https:// to www URLs
        for www_url in www_matches:
            urls.append(f"https://{www_url.strip()}")
        
        return list(set(urls))
    
    def is_bot_mentioned(self, text):
        """Check if bot is mentioned"""
        if not text:
            return False
        return f"@{self.bot_username.lower()}" in text.lower()
    
    def create_archive_url(self, url):
        """Create archive URL using current timestamp format"""
        try:
            # Ensure URL has protocol
            if not url.startswith(('http://', 'https://')):
                url = f"https://{url}"
            
            # Get current timestamp in archive.ph format (YYYYMMDDHHMMSS)
            now = datetime.now()
            timestamp = now.strftime("%Y%m%d%H%M%S")
            
            # Create direct archive URL
            archive_url = f"https://archive.ph/{timestamp}/{url}"
            
            return archive_url
            
        except Exception as e:
            print(f"Error creating archive URL for {url}: {e}")
            return None
    
    def create_simple_archive_url(self, url):
        """Create archive URL that redirects to latest snapshot"""
        try:
            # Ensure URL has protocol
            if not url.startswith(('http://', 'https://')):
                url = f"https://{url}"
            
            # Get current year
            current_year = datetime.now().year
            
            # Create archive URL that redirects to latest snapshot in current year
            archive_url = f"https://archive.ph/{current_year}/{url}"
            
            return archive_url
            
        except Exception as e:
            print(f"Error creating archive URL for {url}: {e}")
            return None
    
    def submit_to_archive(self, url):
        """Submit URL to archive.ph and return the direct archive URL"""
        try:
            print(f"📦 Submitting to archive: {url}")
            
            # Ensure URL has protocol
            if not url.startswith(('http://', 'https://')):
                url = f"https://{url}"
            
            # Submit to archive.ph
            data = {'url': url}
            headers = {
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36',
                'Content-Type': 'application/x-www-form-urlencoded',
                'Origin': 'https://archive.ph',
                'Referer': 'https://archive.ph/'
            }
            
            # Submit the URL (don't follow redirects)
            response = requests.post("https://archive.ph/", data=data, headers=headers, 
                                   allow_redirects=False, timeout=30)
            
            # Create direct archive URL regardless of response
            # This gives users immediate access to the archive
            simple_archive_url = self.create_simple_archive_url(url)
            
            if response.status_code == 302:
                # Got redirect - archive was successful
                redirect_url = response.headers.get('Location')
                if redirect_url and 'archive.ph' in redirect_url:
                    print(f"✅ Archive successful: {redirect_url}")
                    return redirect_url
            
            # Even if we didn't get a redirect, provide the year-based URL (latest snapshot)
            print(f"📁 Providing latest snapshot URL: {simple_archive_url}")
            return simple_archive_url
            
        except Exception as e:
            print(f"❌ Error submitting {url}: {e}")
            # Still provide direct URL as fallback
            return self.create_simple_archive_url(url)
    
    def process_archive_request(self, text, sender_name="User"):
        """Process a message for archive requests"""
        if not self.is_bot_mentioned(text):
            return None
        
        print(f"📨 Processing archive request from {sender_name}")
        
        urls = self.extract_urls(text)
        if not urls:
            return f"@{sender_name} I didn't find any URLs to archive in your message."
        
        archived_results = []
        for url in urls:
            # Submit to archive and get URL
            archived_url = self.submit_to_archive(url)
            if archived_url:
                archived_results.append(f"📁 {url}\n   → {archived_url}")
            else:
                archived_results.append(f"❌ Failed to create archive for: {url}")
        
        reply = f"@{sender_name} Here are your archived links:\n\n" + "\n\n".join(archived_results)
        return reply
    
    def send_archive_reply(self, chat_id, reply_text):
        """Send archive reply using MCP tools"""
        try:
            print(f"💬 Sending archive reply to chat {chat_id}")
            # This will use the actual MCP function
            return True
        except Exception as e:
            print(f"❌ Failed to send reply: {e}")
            return False

def test_direct_archive():
    """Test the direct archive functionality"""
    bot = DirectArchiveBot()
    
    test_urls = [
        "https://example.com",
        "https://github.com",
        "www.google.com",
        "https://news.ycombinator.com"
    ]
    
    print("🧪 Testing Direct Archive URLs")
    print("=" * 40)
    
    for url in test_urls:
        print(f"\n--- Testing: {url} ---")
        
        # Test simple archive URL
        simple_url = bot.create_simple_archive_url(url)
        print(f"Simple archive: {simple_url}")
        
        # Test timestamped archive URL
        timestamp_url = bot.create_archive_url(url)
        print(f"Timestamp archive: {timestamp_url}")
        
        # Test full submission
        result = bot.submit_to_archive(url)
        print(f"Submission result: {result}")

def test_message_processing():
    """Test message processing"""
    bot = DirectArchiveBot()
    
    test_messages = [
        "@Angel_Dimi_Bot please archive https://example.com",
        "@Angel_Dimi_Bot check out https://github.com and www.google.com",
        "Hello @Angel_Dimi_Bot no links here",
        "@Angel_Dimi_Bot https://news.ycombinator.com is interesting"
    ]
    
    print("\n🧪 Testing Message Processing")
    print("=" * 40)
    
    for i, message in enumerate(test_messages):
        print(f"\n--- Test {i+1}: {message[:50]}... ---")
        result = bot.process_archive_request(message, "TestUser")
        if result:
            print("Reply:")
            print(result)
        else:
            print("No reply (bot not mentioned)")

def main():
    """Main function"""
    import sys
    
    bot = DirectArchiveBot()
    
    if len(sys.argv) > 1:
        command = sys.argv[1].lower()
        
        if command == "test-direct":
            test_direct_archive()
            
        elif command == "test-processing":
            test_message_processing()
            
        elif command == "process":
            if len(sys.argv) < 3:
                print("Usage: python direct_archive_bot.py process '<message>'")
                return
            
            message = " ".join(sys.argv[2:])
            result = bot.process_archive_request(message, "User")
            if result:
                print("Bot Reply:")
                print(result)
            else:
                print("No reply needed (bot not mentioned)")
        
        elif command == "archive":
            if len(sys.argv) < 3:
                print("Usage: python direct_archive_bot.py archive <url>")
                return
            
            url = sys.argv[2]
            result = bot.submit_to_archive(url)
            print(f"Archive URL: {result}")
        
        else:
            print(f"Unknown command: {command}")
    
    else:
        print("🤖 Direct Archive Bot")
        print("Available commands:")
        print("  test-direct           - Test direct URL creation")
        print("  test-processing       - Test message processing")
        print("  process '<message>'   - Process a message")
        print("  archive <url>         - Archive a single URL")

if __name__ == "__main__":
    main()